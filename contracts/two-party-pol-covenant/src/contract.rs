...
pub fn instantiate(
    ...
    let party_a_interchain_router_address =
        get_precomputed_address(deps.as_ref(), msg.contract_codes.router_code, &creator_address, PARTY_A_INTERCHAIN_ROUTER_SALT)?;
    let party_b_interchain_router_address =
        get_precomputed_address(deps.as_ref(), msg.contract_codes.router_code, &creator_address, PARTY_B_INTERCHAIN_ROUTER_SALT)?;
...
//                     to_json_binary(&holder_instantiate_tx)?.to_string(),
//                 )
