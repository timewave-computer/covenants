...
fn upload_contracts(test_ctx: &mut TestContext) -> Result<(), LocalError> {
    ...
    uploader
        ...
        .unwrap();
    uploader
        .send_with_local_cache(POLYTONE_PATH, LOCAL_CODE_ID_CACHE_PATH)
        .unwrap();
    uploader
        .with_chain_name(OSMOSIS_CHAIN_NAME)
        .send_with_local_cache(POLYTONE_PATH, LOCAL_CODE_ID_CACHE_PATH_OSMO)
        .unwrap();
...
fn make_pooler(
    ...
    let polytone_note_address = {
        let mut polytone_note = test_ctx.get_contract().contract("polytone_note").get_cw();
        polytone_note
            .instantiate(
                DEFAULT_KEY,
