...
use crate::{setup::{base_suite::{BaseSuite, BaseSuiteMut}, instantiates::clock, ADMIN, DENOM_ATOM_ON_NTRN, DENOM_LS_ATOM_ON_NTRN}, test_native_router::suite::NativeRouterBuilder};
...
fn test_execute_route_balances_with_one_balance() {
    ...
    suite.tick_contract(router.clone())
        .assert_event(&Event::new("wasm")
            .add_attribute("method", "try_route_balances")
...
fn test_execute_route_balances_with_multiple_balances() {
    let mut suite = NativeRouterBuilder::default()
        .with_denoms(vec![DENOM_ATOM_ON_NTRN.to_string(), DENOM_LS_ATOM_ON_NTRN.to_string()])
        .build();
...
fn test_execute_distribute_fallback_validates_explicit_denoms() {
    ...
    suite.distribute_fallback(
        vec![DENOM_ATOM_ON_NTRN.to_string(), DENOM_LS_ATOM_ON_NTRN.to_string()]
    );
}
...
fn test_execute_distribute_fallback_happy() {
    ...
    suite.distribute_fallback(vec![DENOM_LS_ATOM_ON_NTRN.to_string()])
        .assert_event(
            &Event::new("wasm").add_attribute("method", "try_distribute_fallback")
        );
