...
use crate::{
    contract::{
        instantiate, query, reply, CLOCK_REPLY_ID, DEPOSITOR_REPLY_ID, HOLDER_REPLY_ID,
        LP_REPLY_ID, LS_REPLY_ID,
    },
    msg::{InstantiateMsg, PresetIbcFee, Timeouts, DEFAULT_TIMEOUT},
    state::{CLOCK_CODE, DEPOSITOR_CODE, HOLDER_CODE, IBC_FEE, LP_CODE, LS_CODE, TIMEOUTS},
};
...
fn get_init_msg() -> InstantiateMsg {
    InstantiateMsg {
      ...
      preset_lp_fields: covenant_lp::msg::PresetLpFields {
        ...
        expected_ls_token_amount: Uint128::new(500),
        allowed_return_delta: Uint128::new(5),
        expected_native_token_amount: Uint128::new(500),
      },
...
fn test_init() {
    ...
    let ibc_timeout = TIMEOUTS.load(&deps.storage).unwrap();
    assert_eq!(ibc_timeout, Timeouts::default());
    ...
    assert_eq!(
        ...
        WasmMsg::Instantiate {
            ...
            msg: to_binary(&init_msg.clone().preset_lp_fields.to_instantiate_msg(
                ...
                holder_reply_res.contract_address,
    ...
    assert_eq!(
        ...
        WasmMsg::Instantiate {
            ...
            .unwrap(),
            funds: vec![],
            label: init_msg.preset_depositor_fields.label
        }
        .into()
    );
